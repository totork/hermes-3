...
Field3D LowSources::sourceterm(const Field3D ar, const BoutReal val, const BoutReal scale, const BoutReal width) {
  Field3D result = 0.0;
  BOUT_FOR(i, ar.getRegion("RGN_NOY")){
    BoutReal diff = ar[i] - val;
...
LowSources::LowSources(std::string name, Options& alloptions, Solver*)
  ...
  low_temperature_width = options["low_temperature_width"]
  ...
  low_timescale = options["low_timescale"]
                           ...
                           .withDefault<BoutReal>(1e-6) * Omega_ci;
...
void LowSources::transform_impl(GuardedOptions& state) {
  ...
  const Field3D T = species.isSet("temperature")
                        ...
                        : 0.0;
  if (low_density > 0.0) {
    Field3D N_src = LowSources::sourceterm(N, low_density, low_timescale, low_density_width);
  ...
  if (low_temperature > 0.0) {
    Field3D E_src = (3.0 / 2.0) *  N * LowSources::sourceterm(T, low_temperature, low_timescale, low_temperature_width);
